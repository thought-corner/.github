...
| Software Architecture | [rate-limiting-algorithms](https://github.com/thought-corner/rate-limiting-algorithms) | 고정 윈도우 카운터 알고리즘으로 구현한 Redis 기반 분산 레이트 리미터 — 윈도우별 요청 수를 세어 한도 초과 요청을 차단 | **[FIX(고정 카운터 알고리즘 - Redis 커맨드 타임아웃 미설정으로 fail-open이 느린/hang Redis를 방어하지 못하는 문제)](https://github.com/thought-corner/rate-limiting-algorithms/issues/2)** |
