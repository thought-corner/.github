...
| Testing | [test-driven-development](https://github.com/thought-corner/test-driven-development.git) | TDD(Test-Driven Development)로 판매자-구매자 E-커머스 API 명세를 정의하는 문제 — 35개 outside-in 통합 테스트로 회원가입·토큰 발행·상품 거래의 입출력 계약(상태코드·응답·JWT)을 `@TddApiTest`로 명세화 |
